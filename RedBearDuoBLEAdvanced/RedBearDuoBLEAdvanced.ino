...
#define BLE_SHORT_NAME 'L','a','b','3'  

/* Define the pins on the Duo board
 * TODO: change and add/subtract the pins here for your applications (as necessary)
 */
#define DIGITAL_OUT_PIN_R            D0
#define DIGITAL_OUT_PIN_G            D1
#define DIGITAL_OUT_PIN_B            D2
...
void setup() {
   ...
   */
   pinMode(DIGITAL_OUT_PIN_R, OUTPUT);
   pinMode(DIGITAL_OUT_PIN_G, OUTPUT);
   pinMode(DIGITAL_OUT_PIN_B, OUTPUT);
...
int bleReceiveDataCallback(uint16_t value_handle, uint8_t *buffer, uint16_t size) {
  ...
  if (receive_handle == value_handle) {
    ...
    else if (receive_data[0] == 0x0F) { // Command is to control Color
      setLedColors(receive_data[1], receive_data[2], receive_data[3]);
    }
  }
  return 0;
}

static void setLedColors(uint8_t r, uint8_t g, uint8_t b) {
      analogWrite(DIGITAL_OUT_PIN_R, maxColor - r);
      analogWrite(DIGITAL_OUT_PIN_G, maxColor - g);
      analogWrite(DIGITAL_OUT_PIN_B, maxColor - b);  
}
