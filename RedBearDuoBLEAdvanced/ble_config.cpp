...
// small diod for BLE connection indication
#define BLE_INDICATOR_OUT          D9
...
void deviceConnectedCallback(BLEStatus_t status, uint16_t handle) {
  switch (status) {
    case BLE_STATUS_OK:
      ...
      analogWrite(BLE_INDICATOR_OUT, 255);
      break;
    default: break;
  }
}
...
void deviceDisconnectedCallback(uint16_t handle) {
  ...
  analogWrite(BLE_INDICATOR_OUT, 0);
}

void configureBLE() {
  pinMode(BLE_INDICATOR_OUT, OUTPUT);
